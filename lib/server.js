...
drawer.of("/").on("connection", sock => {
  ...
  sock.on(AGGREGATION_ROOM_SUBSCRIBE_EVENT, () => {
    ...
    sock.join(AGGREGATION_ROOM, function(err) {
      ...
      // var initialEvent = iface.toAggregationEvent();
...
// iface.on("aggregation", function(value) {
//   log`iface aggregation event recieved`;
//   drawer.in("aggregation").emit("sunny::aggregation.tick", value);
// });

// iface.on("solo_scrape", function(value) {
//   log`emitting solo_scrape data for ${value.oid}`;
//   drawer.in(PLANT_ROOM_PREFIX + value.oid).emit("sunny::plant.tick", value);
// });
