...
socks.of("/").on("connection", sock => {
  ...
  sock.on("sunny::aggregation.subscribe", () => {
    ...
    sock.join("aggregation", function(err) {
      if (err)
        return log`failed to add ${sock.id} to aggregation room due to err: ${err.message}`;
  ...
  sock.on("sunny::aggregation.unsubscribe", () => {
    ...
    sock.leave("aggregation", function(err) {
      if (err)
        return log`failed to remove ${sock.id} from aggregation room due to err: ${err.message}`;
  ...
  sock.on("sunny::plant.subscribe", ({ oid }) => {
    ...
    sock.join(roomId, function(err) {
      if (err)
        return log`failed to add ${sock.id} to from plant room(${roomId}) room due to err: ${err.message}`;
  ...
  sock.on("sunny::plant.unsubscribe", ({ oid }) => {
    ...
    sock.leave(roomId, function(err) {
      if (err)
        return log`failed to remove ${sock.id} from from plant room(${roomId}) room due to err: ${err.message}`;
