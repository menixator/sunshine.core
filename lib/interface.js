...
class APIInterface {
  ...
  async getPlantPowerReading(oid, date) {
    ...
    let offBy = Math.max(
      (closestFifteenMinute.valueOf() - latest.timestamp) / (15 * 60 * 1000),
      0
    );
  ...
  async aggregate() {
    ...
    aggregated.energy.total = normalize.energy(
      aggregated.energy.total
    );
    aggregated.energy.today = normalize.energy(
      aggregated.energy.today
    );
