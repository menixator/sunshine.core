...
class APIInterface {
  ...
  async aggregate(detailedPlantData = false) {
    let now = Date.now();
    ...
    for (let plant of await plants) {
      ...
      if (detailedPlantData) {
        plantData.details = {
          power: plantPower,
          energy: plantEnergy
        };

        plantData.co2Avoided = {
          total: {
            value: round.round10(
              CARBON_GRAM_AVOIDED_PER_KWH * plantEnergy.total.value,
              -2
            ),
            unit: "g"
          },
          today: {
            value: round.round10(
              CARBON_GRAM_AVOIDED_PER_KWH * plantEnergy.day.value,
              -2
            ),
            unit: "g"
          }
        ...
        plantData.revenue = {
          total: {
            value: round.round10(USD_PER_KWH * plantEnergy.total.value, -2),
            unit: "$"
          },
          today: {
            value: round.round10(USD_PER_KWH * plantEnergy.day.value, -2),
            unit: "$"
          }
        };
      }
      aggregatedPlants.push(plantData);
    }
