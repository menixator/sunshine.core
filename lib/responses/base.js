...
class BaseReponse {
  ...
  findError(node) {
    let error = xpath.select1("error", node);
    if (error) {
  ...
  findFirst(src, haystack) {
    let result = this.find(src, haystack);
    if (!result)
      throw new Error(
        `couldn\'t find an element for ${src} in ${this.ptr(haystack)}`
      );
    return result;
  }
