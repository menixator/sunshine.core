...
class PlantProfile extends Base {
  ...
  parse() {
    ...
    this.peakPower = {
      value: parseFloat(this.findOneText("peak-power", this.contextNode), 10),
      unit: this.attr("unit", "peak-power", this.contextNode)
    };

    this.location = this.findOneText("city-country", this.contextNode);
    this.installedTimestamp = this.findOneText("start-date", this.contextNode);
  ...
  parseChannel(channel, key) {
    ...
    XRegExp.forEach(value, PlantProfile.VALUE_PARSER, (match, idx) => {
      ...
      if (reading.units.length === 2 && reading.units[0] === 'kWh' && reading.units[1] === 'kWp'){
        // Specific Yield
        chan.specificYield = reading
      } else {
    ...
    // Remove readings without a unit
