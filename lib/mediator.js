...
class Mediator extends EventEmitter {
  ...
  async getAggregatedStatistics(statisticType, timestamp, interval) {
    ...
    switch (statisticType) {
      case "co2-avoided":
        converter = CARBON_GRAM_AVOIDED_PER_KWH;
  ...
  async aggregate() {
    ...
    for (let plant of await plants) {
      let reading;
      try {
        reading = await this.getPlantInfo(plant.oid);
      } catch (err) {
        if (this.isLeechPolling && this.focusedPool.has(plant.oid)) {
  ...
  async getPlantEnergy(oid) {
    let lastDataExact = await this.iface.lastDataExact(oid);
