...
class Mediator extends EventEmitter {
  constructor(iface) {
    ...
    this.log`mediator created. will fetch plants every: ${this
      .plantFetchInterval / 1000} seconds. will aggregate every: ${this
      .aggregationInterval / 1000} seconds`;
  }
  ...
  async getSpreadsheetStatistics() {
    ...
    for (let [idx, plant] of plants.entries()) {
      ...
      let todayOverview =
        monthOverview.readings[monthOverview.readings.length - 1];
      let yesterdayOverview =
        monthOverview.readings[monthOverview.readings.length - 2];
      ...
      let row = {
        ...
        yield: {
          today: {
            value: (todayOverview && todayOverview.difference) || 0,
            unit: monthOverview.unit
          },
          yesterday: {
            value: (yesterdayOverview && yesterdayOverview.difference) || 0,
            unit: monthOverview.unit
          },
          month: {
            value:
              (monthOverview.summary && monthOverview.summary.difference) || 0,
            unit: (monthOverview.summary && monthOverview.summary.unit) || null
          },
          year: {
            value:
              (yearOverview.summary && yearOverview.summary.difference) || 0,
            unit: (yearOverview.summary && yearOverview.summary.unit) || null
          },
          total: {
            value: (todayOverview && todayOverview.absolute) || 0,
            unit: monthOverview.unit || null
          }
        },
        specificYield: {
          month: {
            value: monthOverview.summary
              ? monthOverview.summary.difference / peakPower.value
              : 0,
            unit: `${monthOverview.summary && monthOverview.summary.unit}/${
              peakPower.unit
            }`
          },
          year: {
            value: yearOverview.summary
              ? yearOverview.summary.difference / peakPower.value
              : 0,
            unit: yearOverview.summary
              ? `${yearOverview.summary && yearOverview.summary.unit}/${
                  peakPower.unit
                }`
              : null
          }
  ...
  async getPlantStats(
    ...
    let res = await this.iface[
      interval === "month" ? "monthOverview" : "yearOverview"
    ](oid, timestamp.valueOf());
    ...
    for (let plant of await plants) {
      ...
      try {
        ...
        if (
          realTime &&
          this.isLeechPolling &&
          this.focusedPool.has(plant.oid)
        ) {
      ...
      if (realTime)
        this.log`${plant.name} power.now = ${power.latest.value} kW (${
          power.discontinuity > 0 ? -power.discontinuity : "@@"
        })`;
