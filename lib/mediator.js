...
const { CARBON_GRAM_AVOIDED_PER_KWH, USD_PER_KWH } = require("./interface");
...
class Mediator extends EventEmitter {
  ...
  async getPlantMonthlyStats(stats, oid, timestamp, interval) {
    if (["energy", "co2Avoided", "revenue"].indexOf(stats) === -1)
      throw new Error("unknown statistic " + stats);
    timestamp = moment(timestamp || new Date());

    let res = await this.iface.monthOverview(oid, timestamp.valueOf());
    let converter = null;
    let convertedUnit = null;
    let namespace = "plant.energyReadings";

    switch (stats) {
      case "co2Avoided":
        converter = CARBON_GRAM_AVOIDED_PER_KWH;
        namespace = "plant.co2Avoided";
        convertedUnit = "g";
        break;

      case "revenue":
        namespace = "plant.revenue";
