...
class Sunshine extends EventEmitter {
  ...
  async getPlantPowerReading(oid) {
    ...
    if (dayOverview.readings.length === 0)
      throw new Error("no readings for today from oid: " + oid);
    ...
    let offBy = (closestFifteenMinute - latestReadingMoment)/(15 * 60 * 1000)


    return {
      oid,
      latest: { value: latest.mean, unit: dayOverview.unit },
      timestring: dayOverview.timestring + " " + latest.timestring, 
      offBy
    };
  }
  ...
  async getPlantEnergy(oid) {
    ...
    return {
      oid,
      total: {
        value: lastDataExact.day.absolute,
        unit
      },
      day: {
        value: lastDataExact.day.difference,
        unit
      },
      hour: {
        value: lastDataExact.hour.difference,
        unit
      }
    };
  }
